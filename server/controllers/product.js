...
exports.deleteProduct = function(req, res){
	...
	Product.findById(req.params.id)
		  ...
		  .exec(function(err,foundProduct){
			...
			if(foundProduct.shoppingCarts.length >0){
...
exports.createProduct = function(req,res){
	const {  name, category, image, shared, quantity, description, price } = req.body;
	const user = res.locals.user;

	const product = new Product({  name,  category, image, shared, quantity, description, price });
	product.user = user;
